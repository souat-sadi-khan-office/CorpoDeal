    ...
    <form action="{{ route("admin.currency.update", $model->id) }}" method="POST" class="ajax-form">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mt-3 form-group">
                <label for="decimal_separator">Status <span class="text-danger">*</span></label>
                <select name="decimal_separator" id="decimal_separator" class="form-control select" required>
                    <option {{ $model->decimal_separator == 0 ? 'selected' : '' }} value="0">0 Decimal Number</option>
                    <option {{ $model->decimal_separator == 1 ? 'selected' : '' }} value="1">1 Decimal Number</option>
                    <option {{ $model->decimal_separator == 2 ? 'selected' : '' }} value="2">2 Decimal Number</option>
                    <option {{ $model->decimal_separator == 3 ? 'selected' : '' }} value="3">3 Decimal Number</option>
                    <option {{ $model->decimal_separator == 4 ? 'selected' : '' }} value="4">4 Decimal Number</option>
                    <option {{ $model->decimal_separator == 5 ? 'selected' : '' }} value="5">5 Decimal Number</option>
                </select>
            </div>

...
@section('title', 'Update Carrier Information | Carrier Management')
@push('style')
...
@section('page_name')
    <div class="app-content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="h3 mb-0">Update Carrier Information</h1>
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Update Carrier Information</li>
                    </ol>
                </div>
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <div class="row">
                <form action="{{ route("admin.carrier.update", $model->id) }}" enctype="multipart/form-data" method="POST" class="content_form">
                    @csrf
                    @method('PATCH')
                    <div class="row">
                        ...
                        <div class="col-md-6 mt-3 form-group">
                            ...
                            <input type="text" name="transit_time" id="transit_time" class="form-control" required value="{{ $model->transit_time }}">
                        </div>

                        <div class="col-md-6 mt-3 form-group">
                            <label for="tracking_url">Tracking URL</label>
                            <input type="url" name="tracking_url" id="tracking_url" class="form-control" value="{{ $model->tracking_url }}">
                        </div>

                        <div class="col-md-12 mt-3 form-group">
                            <label for="logo">Logo</label>
                            <input type="file" name="logo" id="logo" class="form-control dropify" data-default-file="{{ $model->logo ? asset($model->logo) : '' }}">
                        </div>
                        ...
                        <div class="col-md-12 mt-3 form-group">
                            ...
                            <select name="free_shipping" id="free_shipping" class="form-control select" required>
                                <option {{ $model->free_shipping == '0' ? 'selected' : '' }} value="0">No</option>
                                <option {{ $model->free_shipping == '1' ? 'selected' : '' }} value="1">Yes</option>
                            </select>
                        </div>

                        <div style="display: {{ $model->free_shipping == 1 ? 'none' : 'block' }}" id="not_free_shipping_area">
                            ...
                            <div class="col-md-12 form-group mt-3">
                                ...
                                <select name="rule_type" id="rule_type" class="form-control select" required data-parsley-errors-container="#rule_type_error">
                                    <option {{ $model->rule_type == 'fixed' ? 'selected' : '' }} value="fixed">Fixed Type</option>
                                    <option {{ $model->rule_type == 'weight' ? 'selected' : '' }} value="weight">According to Weight</option>
                                    <option {{ $model->rule_type == 'price' ? 'selected' : '' }} value="price">According to Price</option>
                                    <option {{ $model->rule_type == 'quantity' ? 'selected' : '' }} value="quantity">According to Quantity</option>
                                </select>
                            </div>
                            ...
                            <div id="price_range_form">
                                ...
                                <div class="col-md-12 table-responsive">
                                    @php
                                        $counter = 0;
                                        foreach ($model->rules->groupBy('country_id') as $key => $value) {
                                            $counter = count($value);
                                        }
                                    @endphp
                                            ...
                                            @if ($model->rules)
                                                    ...
                                                    @for ($i = 0; $i < $counter; $i++)
                                                        <td>
                                                            <div class="input-group mb-2">
                                                                ...
                                                                <input type="number" class="form-control" name="delimiter1[]" value="" step="0.01">
                                                            </div>
                                                        </td>
                                                    @endfor
                                                ...
                                                @foreach ($countries as $country)
                                                    <tr>
                                                        ...
                                                        @for ($i = 0; $i < $counter; $i++)
                                                            @php
                                                                $model->rules->where('country_id', $country->id)
                                                            @endphp
                                                            <td>
                                                                <div class="input-group mb-2">
                                                                    <div class="input-group-prepend">
                                                                    ...
                                                                    <input type="number" class="form-control shipping_cost" name="carrier_price[{{ $country->id }}][]" placeholder="Cost" disabled="" value="">
                                                                </div>
                                                            </td>
                                                        @endfor
                                                    </tr>

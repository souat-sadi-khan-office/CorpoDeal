...
class DeductPointsJob implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            DB::transaction(function () use ($products, $productDetails, $user, &$totalPoints) {
                ...
                $user->decrement('points', $totalPoints); // Deduct points in one operation
            });

            Helpers::activity($user->id, null, null, 'system', "{$totalPoints} Points deducted successfully for order ID: {$this->orderId} and user: {$user->name}", null);
        } catch (\Exception $e) {
            Helpers::activity($this->userId, null, null, 'system', "Error in DeductPointsJob for order ID: {$this->orderId} and user ID: {$this->userId}: " . $e->getMessage(), null);
            throw $e;
        }
    }
